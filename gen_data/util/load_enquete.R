...
##################INFOS GENERALES####################################
## Création d'un sous-jeu de données correspondant aux informations générales
## sur la parcelle
...
colnames(fich_general)[1:3] <-
  c("ID_Parcelle", "ID_Exploitation", "Année_SuiviParcelle")
...
Materiel <-
  paste(fich_farm$Matériel_Exploitation_Type,
        fich_farm$Matériel_Largeur..en.m.,
        fich_farm$Matériel_Puissance..en.cv.,
        fich_farm$Matériel_Conso..en.l.heure.,
        fich_farm$Matériel_Débit..ha.heure,
        sep="_")
...
fich_semis$Date_Récolte_Inter <-
  lapply(fich_semis$Date_Récolte_Inter, strptime, "%d/%m/%Y")
...
# ajout des variables calculées dans le tableau fich_Wsol
...
############################PHYTOSANITAIRES#########################################
##Création d'un sous-jeu de données correspondant aux informations sur les
...
Surf_traitH <-
  xtabs(as.numeric(as.character(Surface_traitée_ha)) ~ ID_Parc_Tri + Produit_phyto,
        data=fich_Herbi) 
...
info_Insect <-
  data.frame(ID_Parc_Tri          = as.character(info_Insect[, 1]),
             ...
             Surface_traitée_ha   = funlist_multi(info_Insect$Surface_traitée_ha,
                                                  missing.data = 0),
...
info_Fongi <-
  data.frame(ID_Parc_Tri          = as.character(info_Fongi[, 1]),
             ...
             Surface_traitée_ha   = funlist_multi(info_Fongi$Surface_traitée_ha,
                                                   missing.data = 0),
