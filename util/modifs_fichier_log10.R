...
{
  if (icas != 30) {
    for (iparc in unique(interm2.dat$Par))
    {
      print(c(iparc,icas))
      du.dat <- interm2.dat[interm2.dat$Par==iparc,]
      k <- k+1
      ...
      if(!is.na(ww[1])&!is.na(ww[2])) {
        selocc <- (du.dat$Crop.Analyses[1]=="prairie") |
          (du.dat$Crop.Analyses[1,]=="luzerne") |
      ...
      interm2.dat[interm2.dat$Par==iparc,] <- du.dat
    }
  }
