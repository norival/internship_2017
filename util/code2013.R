#################################################################
# Script pour calculer les richesses à 20m² sur les données 2013
#
# Sabrina Gaba
# crée le 6 juillet 2016
# modifié le 16/09/2016 par Lucie Mahaut
#################################################################

#####Charge le fichier des relevés de flore 2013
data2013=read.csv("data/raw/monitoring2013.csv", sep=";", dec= "," , 
                 stringsAsFactors=FALSE,h=T,
                 encoding = "latin1")
#vérification des données
dim(data2013) #dimension du fichier de donnees
head(data2013) #premieres lignes
summary(data2013) #synthese des donnees
var <- colnames(data2013)

unique(data2013$Crop.Analyses) #céréale/Cereal, colza/Colza,Maïs/maïs
data2013$Crop.Analyses[data2013$Crop.Analyses=="Cereal"]="céréale"
data2013$Crop.Analyses[data2013$Crop.Analyses=="Maïs"]="maïs"
...
data2013=modifs_fichier(tab=data2013)

##pb with "6340-6918" field
#data2013<-data2013[-which(data2013$Par=="6340-6918"),]
#############################################################
...
weeds2013 <- data2013[, kept ]
colnames(weeds2013)
weeds2013C<-subset(weeds2013,weeds2013$Crop.Analyses!="friche")
weeds2013C<-subset(weeds2013,weeds2013$Crop.Analyses!="luzerne")
weeds2013C<-subset(weeds2013C,weeds2013C$Crop.Analyses!="prairie")
weeds2013C<-subset(weeds2013C,weeds2013C$Crop.Analyses!="trèfle")

#weeds2013C$Crop.Analyses[weeds2013C$pt==1]
unique(weeds2013C$pt)
...
#"LONG"
###colnames(weeds2013C)
###length(unique(weeds2013C$carre.parc))
## 183 parcelles

###unique(weeds2013C$Par.interf)
#[1] "In"  "pa" "in" "Pa" 
weeds2013C$Par.interf[weeds2013C$Par.interf=="In"]="in"
weeds2013C$Par.interf[weeds2013C$Par.interf=="Pa"]="pa"

###length(unique(weeds2013C$Crop.Analyses))
## 5 cultures

###length(unique(weeds2013$Espèce_origin))
## 294 espèces soit 294 colonnes dans la matrice

weeds2013C1 <- cbind(weeds2013C, as.factor(weeds2013C$Espece_origin))
...
###########################################
#weeds<-weeds2013C1
weeds<-read.csv("data/generated/weeds2013.csv", sep = ";",dec=",", encoding = "latin1")
...
write.table(test, "data/generated/transpose_abondance_per_fieldcore2013.csv", sep = ";")
...
write.table(test, "data/generated/transpose_abondance_per_field2013.csv", sep = ";")
...
A=read.csv("data/generated/transpose_abondance_per_fieldcore2013.csv", sep = ";",h=T)
...
A_Diversity$Year=rep(2013,length(A_Diversity[,1]))
...
mat2013 = xtabs(Richness~ carre.parc, A_Diversity)

A_Diversity_obs=A_Diversity

##Etape 1: estimation de la richesse observée sur 40 m²
A=read.csv("data/generated/transpose_abondance_per_quadrat2013.csv", sep = ";",h=T)
...
write.table(A_Diversity, "data/generated/Diversity_fieldcore2013.csv", sep = ";")
...
data2013.dat <- read.csv("data/generated/transpose_abondance_per_quadrat2013.csv",
...
mat_vide <- matrix(Inf,
                   ncol = length(unique(data2013.dat$sp)),
                   nrow = length(unique(data2013.dat$carre.parc)))
abond_per_plot <- as.data.frame(mat_vide)

colnames(abond_per_plot) <- unique(data2013.dat$sp)
rownames(abond_per_plot) <- unique(data2013.dat$carre.parc)

for (parc in unique(data2013.dat$carre.parc)) {

  # On récupère les lignes pour la parcelle parc
  dat_sub <- data2013.dat[data2013.dat$carre.parc == parc, ]
...
write.csv(abond_per_plot, "data/generated/abondt_per_plot_2013.csv",
          row.names = FALSE)
