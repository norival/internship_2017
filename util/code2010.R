#################################################################
# Script pour calculer les richesses à 20m² sur les données 2010
#
# Sabrina Gaba
...
#####Charge le fichier des relevés de flore 2010
data2010=read.csv("monitoring2010.csv", sep=";", dec= "," , 
                  stringsAsFactors=FALSE,h=T)
#vérification des données
dim(data2010) #dimension du fichier de donnees 24787L & 31C
head(data2010) #premieres lignes
summary(data2010) #synthese des donnees
var <- colnames(data2010)

unique(data2010$Crop.Analyses) #céréale/Cereal, colza/Colza,Maïs/maïs

###Correction des noms d'espèces adventices
##code Joël
source("modifs_fichier_log10.R")
data2010=data2010[data2010$No_parcelle!="ZPS197-2010",]
data2010=modifs_fichier(tab=data2010)

##pb with "6340-6918" field
#data2010<-data2010[-which(data2010$Par=="6340-6918"),]
#############################################################
...
weeds2010C<-subset(weeds2010,weeds2010$Crop.Analyses!="luzerne")
weeds2010C<-subset(weeds2010C,weeds2010C$Crop.Analyses!="prairie")
weeds2010C<-subset(weeds2010C,weeds2010C$Crop.Analyses!="trèfle")

#weeds2010C$Crop.Analyses[weeds2010C$pt==1]
...
#"LONG"
###colnames(weeds2010C)
###length(unique(weeds2010C$carre.parc))
## 183 parcelles

###unique(weeds2010C$Par.interf)
#[1] "In"  "pa" "in" "Pa" 
weeds2010C$Par.interf[weeds2010C$Par.interf=="In"]="in"
weeds2010C$Par.interf[weeds2010C$Par.interf=="Pa"]="pa"

###length(unique(weeds2010C$Crop.Analyses))
## 5 cultures

###length(unique(weeds2010$Espèce_origin))
...
write.table(weeds2010C1, "Data-Prog/weeds2010.csv", sep = ";")

###########################################
## Aggregation des especes
###########################################
#weeds<-weeds2010C1
weeds<-read.csv("Data-Prog/weeds2010.csv", sep = ";",dec=",")
...
#head(A, 25)
write.table(A, "Data-Prog/transpose_abondance_per_quadrat2010.csv", sep = ";")
...
write.table(test, "Data-Prog/transpose_abondance_per_fieldcore2010.csv", sep = ";")
...
write.table(test, "Data-Prog/transpose_abondance_per_field2010.csv", sep = ";")
...
A=read.csv("Data-Prog/transpose_abondance_per_fieldcore2010.csv", sep = ";",h=T)
...
##Etape 1: estimation de la richesse observée sur 40 m²
A=read.csv("Data-Prog/transpose_abondance_per_quadrat2010.csv", sep = ";",h=T)
...
write.table(A_Diversity, "Data-Prog/Diversity_fieldcore2010.csv", sep = ";")
