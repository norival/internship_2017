...
##code Joël
source("util/modifs_fichier_log10.R",
       encoding = "latin1")
...
data2010.dat <- read.csv("data/generated/transpose_abondance_per_quadrat2010.csv",
...
mat_vide <- matrix(Inf,
                   ncol = length(unique(data2010.dat$sp)),
                   nrow = length(unique(data2010.dat$carre.parc)))
abond_per_plot <- as.data.frame(mat_vide)

colnames(abond_per_plot) <- unique(data2010.dat$sp)
rownames(abond_per_plot) <- unique(data2010.dat$carre.parc)

for (parc in unique(data2010.dat$carre.parc)) {

  # On récupère les lignes pour la parcelle parc
  dat_sub <- data2010.dat[data2010.dat$carre.parc == parc, ]
...
write.csv(abond_per_plot, "data/generated/abondt_per_plot_2010.csv",
          row.names = FALSE)
